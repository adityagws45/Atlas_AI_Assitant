...
# Ensure new apps (e.g. drive) are migrated on the local sqlite used by this suite
try:
...
def test_compare_and_telegram_flow() -> None:
    ...
    assert (
        "loaded" in r1.lower()
        or "got it" in r1.lower()
        or "document ready" in r1.lower()
        or "ready to answer" in r1.lower()
    )
